...
import { useAuth } from '../contexts/AuthContext.jsx'; // Updated to .jsx
...
export const useVoiceMode = (projectId = null) => {
  ...
  // **** IMPORTANT: This URL should match your deployed voiceModeHandler Cloud Function ****
  const VOICE_MODE_API_URL = 'https://us-central1-raptor-suite.cloudfunctions.net/voiceModeHandler';
  // ***********************************************************************************
  ...
  // Helper function to convert ArrayBuffer to Base64 in browser
  ...
  // Helper function to convert Base64 to ArrayBuffer in browser
  ...
  const processAudioInput = useCallback(async (audioBlob) => {
    ...
    try {
      // Convert audio Blob to base64 string using browser-compatible function
      const arrayBuffer = await audioBlob.arrayBuffer();
      const base64Audio = arrayBufferToBase64(arrayBuffer); // Use the new helper function
      ...
      if (result.success) {
        ...
        if (result.audioResponse) {
          // Convert base64 audio string back to ArrayBuffer using new helper
          const audioArrayBuffer = base64ToArrayBuffer(result.audioResponse);
          const audioBlob = new Blob([audioArrayBuffer], { type: 'audio/mp3' }); // Ensure type matches what TTS outputs
  ...
  // Play audio response from Raptor (renamed from `playAudioResponse` since integrated above)
  // Re-adding it here just in case any other part of your code directly calls it externally,
  // but the core logic for playing audio is now within processAudioInput's success branch.
